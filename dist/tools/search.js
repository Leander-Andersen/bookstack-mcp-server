    ...
    createSearchTool() {
        return {
            ...
            handler: async (params) => {
                ...
                const searchParams = { query: finalQuery };
                if (params.page)
                ...
                if (f.tag) {
                    ...
                    const filtered = results.data.filter((item) => (item.tags ?? []).some((t) => (!tagName || t.name?.toLowerCase() === tagName) &&
                ...
                const enriched = await Promise.all(results.data.map(async (item) => {
                    if (item.type !== 'page')
                        return item;
                    try {
